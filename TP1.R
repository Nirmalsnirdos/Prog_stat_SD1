...
# 7. Anime note > 9/10 OU plus de 400 000 votes
anime_hype = subset(dfAnime, Score > 9 | Vote > 400000)
nrow(anime_hype)


# EXERCICE 4 - Fusion et Exportation

# 1. Conservation des colonnes : Title, Score, Vote, Ranked
dfAnime = dfAnime[ , c("Title", "Score", "Vote", "Ranked")]
dfManga = dfManga[ , c("Title", "Score", "Vote", "Ranked")]

# 2. Création de la colonne Type
dfAnime$Type = "Anime"
dfManga$Type = "Manga"

# 3. Compilation des dataframes (rbind)
dfConcat = rbind(dfAnime, dfManga)
View(dfConcat)

# 4. Exportation en CSV
write.table(x = dfConcat, 
            file = "C:/Users/nirma/Downloads/ExportTp1.csv",
            sep = ";", 
            row.names = FALSE)
